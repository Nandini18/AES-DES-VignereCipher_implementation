...
def getKeySchedule(key):
    ...
    for i in range(4,44):
        x=[0]*4 #array of size 4
        for j in range(4):
            x[j]=key_schedule[i-4][(j+3)%4]
        key_schedule.append(x)
    #print (key_schedule)
...
def encrypt(plaintext, key_schedule):
    ...
    ADD_ROUND_KEY(state_array,key_schedule[0:4])
    for i in range(1,11):
        SUBSTITUTE_BYTES(state_array)
        SHIFT_ROWS(state_array)
        ADD_ROUND_KEY(state_array,key_schedule[i*4:(i*4)+4])
